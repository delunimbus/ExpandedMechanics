...
function Needle:infect(other)
    ...
    self.sprite:setAnimation(function(sprite, wait)
        ...
        local tension_chance = love.math.random(1, 5)

        local bullet = nil
        if tension_chance >= 4 then
            bullet = self.wave:spawnBullet("virovirokun/virustension", self.x, self.y)
        else
            bullet = self.wave:spawnBullet("virovirokun/virus", self.x, self.y)
        end
