...
function DiamondsAim:onStart()
    self.timer:every(2/5, function()
		...
		self.timer:script(function(wait)
			...
			wait(1/3)
			local poison_chance = love.math.random(1, 5)
			local bullet2 = nil
			--if poison_chance == 5 then
        		--bullet2 = self:spawnBullet("rudinn/diamond_poison", bullet.x, bullet.y, bullet.rotation)
			--else
				bullet2 = self:spawnBullet("rudinn/diamond_white", bullet.x, bullet.y, bullet.rotation)
			--end
