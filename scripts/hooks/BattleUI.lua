...
-------------------------------------------------------------------------------------------------------------
        ...
        if current_item.tp and current_item.tp ~= 0 and current_item.resource == ("tension" or "none") then
            ...
            if current_item.mp and current_item.mp ~= 0 and current_item.resource == "mana" then
                local text_offset = 0
                if current_item.spenders then
                    --Game.battle:updateSplitCost()
                    --local any_spender_present = false
                    --local party_index = nil
                    --local spender = nil
                    for _,spender in ipairs(current_item.spenders) do
                        local party_member = Registry.createPartyMember(spender)
                        local party_index = Game.battle:getPartyIndex(spender)
                        local spender_battler = Game.battle.party[party_index]

                        --spender = Game.party[party_index]
                        if (not spender_battler) or (not spender_battler:isActive()) then
                            Draw.setColor(COLORS.gray)
                        end
                        --[[if spender_battler and battler:isActive() == false then
                            
                        end]]
                        local ox, oy = party_member:getHeadIconOffset()
                        Draw.draw(Assets.getTexture(party_member:getHeadIcons() .. "/head"), text_offset + 260 + 240 + ox, 50 + (tp_offset * 32) + oy)
                        Draw.setColor()
                        text_offset = text_offset + 30
                    end
                    --if #current_item.spenders == 1 then
                        --Draw.setColor(spender.chara:getXActColor())
                    --else Draw.setColor(COLORS["white"]) end
                --else

                end
                Draw.setColor(0, 191/255, 1)
                --print(current_item.mp)
                love.graphics.print(current_item.mp .. " MP", text_offset + 260 + 240, 50 + (tp_offset * 32))
            elseif (current_item.stock and current_item.resource == "stock") or current_item.resource == "stock_show" then
                Draw.setColor(COLORS["ltgray"])
                love.graphics.print(current_item.stock .. "/" .. current_item.stock_limit .. " STCK", 260 + 240, 50 + (tp_offset * 32))
            elseif current_item.hp_cost and current_item.hp_cost ~= 0 and current_item.resource == "health" then
                Draw.setColor(COLORS["red"])
                love.graphics.print(current_item.hp_cost .. " HP", 260 + 240, 50 + (tp_offset * 32))
            end
    ...
    elseif Game.battle.state == "ENEMYSELECT" or Game.battle.state == "XACTENEMYSELECT" then
        ...
        for index = page_offset+1, math.min(page_offset+3, #enemies) do
            ...
            if enemy then
                ----------------------------------------------------------------
                --Draw.setColor(1, 1, 1)
                --love.graphics.print("Lv. " .. enemy.level, 0, 45 + y_off)
                ----------------------------------------------------------------
