...
function DarkEquipMenu:getPassivePreview()
    local passive_preview = {}
    local party = self.party:getSelected()
    local passive = party.passive
    if passive then
        passive = Mod:createPassive(passive)
        passive_preview = { name = passive:getName(), icon = passive.passive_icon, color = passive.passive_color }
    end
...
function DarkEquipMenu:drawStats()
    ...
    self:drawStatPreview("attack", 48, 118, stats, compare, self:getCurrentItemType() == "weapons")
    self:drawStatPreview("magic", 168, 118, stats, compare, false)
    self:drawStatPreview("defense", 48, 143, stats, compare, false)
    self:drawStatPreview("spirit", 168, 143, stats, compare, false)

    local abilities, ability_comp = self:getAbilityPreview()
    local passive = self:getPassivePreview()
    for i = 1, 3 do
        self:drawAbilityPreview(i, -8, 148 + (27 * i), abilities, ability_comp)
    end
...
function DarkEquipMenu:drawPassivePreview(index, x, y, passive--[[, compare]])
