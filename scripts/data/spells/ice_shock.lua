...
---------------------------------
    self.hp_cost_flat = 16

    self.mana_cost = 16

    self.draw_resistance = 5
---------------------------------
...
-----------------------------------------------------
        damage = self:getDrawCastValue(damage)
        --if user.chara:getSpellResourceType(self) == "mana" then
        damage = self:applyPassiveDamageMod(user.chara, damage)
        --end
-----------------------------------------------------
...
function spell:getDamage(user, target)
    ...
    return math.ceil((min_magic * 30) + 90 + Utils.random(10) - (target.spirit * 3))
end
