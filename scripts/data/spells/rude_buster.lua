...
------------------------------------
    ...
    self.hp_cost_flat = 60

    self.draw_resistance = 24
------------------------------------
...
function spell:getDamage(user, target, pressed)
    local damage = math.ceil((user.chara:getStat("magic") * 5) + (user.chara:getStat("attack") * 11) - (target.spirit * 3))
    if pressed then
        damage = damage + 30
    end
    return damage
